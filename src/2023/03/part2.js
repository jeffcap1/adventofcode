...
function createMovement(grid) {
	return {
		left: (x, y) => x - 1 >= 0 ? [x - 1, y, grid[y][x - 1]] : undefined,
		right: (x, y) => x + 1 < grid[y].length ? [x + 1, y, grid[y][x + 1]] : undefined,
		up: (x, y) => y - 1 >= 0 ? [x, y - 1, grid[y - 1][x]] : undefined,
		down: (x, y) => y + 1 < grid.length ? [x, y + 1, grid[y + 1][x]] : undefined,
		upLeft: (x, y) => y - 1 >= 0 && x - 1 >= 0 ? [x - 1, y - 1, grid[y - 1][x - 1]] : undefined,
		upRight: (x, y) => y - 1 >= 0 && x + 1 < grid[y].length ? [x + 1, y - 1, grid[y - 1][x + 1]] : undefined,
		downLeft: (x, y) => y + 1 < grid.length && x - 1 >= 0 ? [x - 1, y + 1, grid[y + 1][x - 1]] : undefined,
		downRight: (x, y) => y + 1 < grid.length && x + 1 < grid[y].length ? [x + 1, y + 1, grid[y + 1][x + 1]] : undefined,
	};
}
